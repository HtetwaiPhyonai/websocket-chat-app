<template>
    <div class="min-h-screen flex flex-col items-center py-4 px-4">

        <!-- Main Chat Interface -->
        <div class="w-full max-w-6xl gap-4">

            <!-- Main Chat Area -->
            <Card class="bg-white/95 backdrop-blur-sm shadow-2xl">

                <template #title>
                    <ChatRoomSideBar :roomOptions="roomOptions" :current-user="currentUser" :switch-room="switchRoom"
                        :room-name="roomName" />
                    <div class="flex items-center justify-between p-4">
                        ...
                        <div class="flex items-center gap-2">
                            <Button icon="pi pi-refresh" @click="fetchMessages" :loading="loadingMessages"
                ...
                <template #content>
                    <div class="px-4 pb-4">
                        ...
                        <!-- Chat Messages -->
                        <div v-else ref="chatContainer"
                            class="h-96 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-gray-50 to-white rounded-lg border border-gray-200">
                            <!-- Empty state -->
                            ...
                            <div v-for="(msg, index) in messages" :key="msg.id || index"
                                class="flex items-end transition-all duration-300 ease-in-out" :class="isCurrentUser(msg.sender)
                                    ? 'justify-end'
                                    : 'justify-start'
                                    ">
                                <div class="flex items-end space-x-2 max-w-xs lg:max-w-md" :class="isCurrentUser(msg.sender)
                                    ? 'flex-row-reverse space-x-reverse'
                                    : ''
                                    ">
                                    <div class="relative">
                                        <Avatar :label="getUserInitial(
                                    ...
                                    <div class="relative group">
                                        ...
                                        <div v-if="isCurrentUser(msg.sender)"
                                            class="text-xs text-gray-400 mt-1 text-right">
                                            <i class="pi pi-clock" v-if="msg.status === 'sending'"></i>
                                            <i class="pi pi-check" v-else-if="
                                                msg.status === 'sent'
                                            "></i>
                                            <i class="pi pi-check-circle text-blue-500" v-else-if="
                                                msg.status === 'delivered'
                                            "></i>
                                            <i class="pi pi-check-circle text-green-500" v-else-if="
                                                msg.status === 'read'
                                            "></i>
                                        </div>
                                    </div>
                        ...
                        <div class="mt-4">
                            <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-200">
                                ...
                                <InputText v-model="newMessage" @keyup.enter="sendMessage" :disabled="loadingMessages"
                                    placeholder="Type your message..."
                                    class="flex-1 border-0 bg-transparent focus:ring-0" />

                                <Button icon="pi pi-send" @click="sendMessage" :disabled="!newMessage.trim() || loadingMessages
